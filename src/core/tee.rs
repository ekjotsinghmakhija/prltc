...
fn get_tee_dir(config: &Config) -> Option<PathBuf> {
    // Env var override
    if let Ok(dir) = std::env::var("PRLTC_TEE_DIR") {
...
pub fn tee_raw(raw: &str, command_slug: &str, exit_code: i32) -> Option<PathBuf> {
    // Check PRLTC_TEE=0 env override (disable)
    if std::env::var("PRLTC_TEE").ok().as_deref() == Some("0") {
        return None;
    }
...
mod tests {
    ...
    #[test]
    fn test_format_hint() {
        let path = PathBuf::from("/tmp/prltc/tee/123_cargo_test.log");
...
"#;
        ...
        assert_eq!(config.directory, Some(PathBuf::from("/tmp/prltc-tee")));
    ...
    fn test_force_tee_hint_respects_env_disable() {
        // When PRLTC_TEE=0, force_tee_hint should return None
        std::env::set_var("PRLTC_TEE", "0");
        let large_output = "x".repeat(1000);
        let hint = force_tee_hint(&large_output, "test_cmd");
        std::env::remove_var("PRLTC_TEE");
        assert!(hint.is_none(), "Should respect PRLTC_TEE=0");
    }
}
