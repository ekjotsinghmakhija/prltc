...
///   1. `.prltc/filters.toml`              — project-local, committable with the repo
///   2. `~/.config/prltc/filters.toml`     — user-global, applies to all projects
...
///
/// `prltc init` generates a commented template for both levels (project or global).
///
/// Environment variables:
///   - `PRLTC_NO_TOML=1`     — bypass TOML engine entirely
///   - `PRLTC_TOML_DEBUG=1`  — print which filter matched and line counts to stderr
///
...
use super::constants::{FILTERS_TOML, PRLTC_DATA_DIR};
...
impl TomlFilterRegistry {
    ...
    fn load() -> Self {
        ...
        // Priority 2: user-global ~/.config/prltc/filters.toml
        if let Some(config_dir) = dirs::config_dir() {
            let global_path = config_dir.join(PRLTC_DATA_DIR).join(FILTERS_TOML);
            if let Ok(content) = std::fs::read_to_string(&global_path) {
                match Self::parse_and_compile(&content, "user-global") {
                    Ok(f) => filters.extend(f),
                    Err(e) => eprintln!("[prltc] warning: {}: {}", global_path.display(), e),
                }
            }
        }
        ...
        match Self::parse_and_compile(builtin, "builtin") {
            Ok(f) => filters.extend(f),
            Err(e) => eprintln!("[prltc] warning: builtin filters: {}", e),
        }
    ...
    fn parse_and_compile(content: &str, source: &str) -> Result<Vec<CompiledFilter>, String> {
        ...
        for (name, def) in file.filters {
            match compile_filter(name.clone(), def) {
                ...
                Err(e) => eprintln!("[prltc] warning: filter '{}' in {}: {}", name, source, e),
            }
        }
        Ok(compiled)
    }
...
fn compile_filter(name: String, def: TomlFilterDef) -> Result<CompiledFilter, String> {
    ...
    for cmd in RUST_HANDLED_COMMANDS {
        if match_regex.is_match(cmd) {
            eprintln!(
                "[prltc] warning: filter '{}' match_command matches '{}' which is already \
...
// ---------------------------------------------------------------------------
// prltc verify — inline test execution
// ---------------------------------------------------------------------------
...
fn collect_test_outcomes(
    ...
    let file: TomlFilterFile = match toml::from_str(content) {
        Ok(f) => f,
        Err(e) => {
            eprintln!("[prltc] warning: TOML parse error during verify: {}", e);
            return;
        }
    };
    ...
    for (name, def) in file.filters {
        ...
        match compile_filter(name.clone(), def) {
            ...
            Err(e) => eprintln!("[prltc] warning: filter '{}' compilation error: {}", name, e),
        }
    }
    ...
    for (filter_name, tests) in file.tests {
        ...
        let compiled = match compiled_filters.get(&filter_name) {
            ...
            None => {
                eprintln!(
                    "[prltc] warning: [[tests.{}]] references unknown filter",
                    filter_name
                );
                continue;
            }
...
pub fn find_matching_filter(command: &str) -> Option<&'static CompiledFilter> {
    if std::env::var("PRLTC_TOML_DEBUG").is_ok() {
        eprintln!(
            "[prltc:toml] looking up filter for: {:?} ({} filters loaded)",
            command,
    ...
    if std::env::var("PRLTC_TOML_DEBUG").is_ok() {
        match result {
            Some(f) => eprintln!("[prltc:toml] matched filter: '{}'", f.name),
            None => eprintln!("[prltc:toml] no filter matched — passthrough"),
        }
    }
    result
}
