...
/// Parsed arguments from either native find or PRLTC find syntax.
#[derive(Debug)]
struct FindArgs {
    pattern: String,
    path: String,
...
/// Native find flags that PRLTC cannot handle correctly.
...
/// Parse arguments from raw args vec, supporting both native find and PRLTC syntax.
///
/// Native find syntax: `find . -name "*.rs" -type f -maxdepth 3`
/// PRLTC syntax: `find *.rs [path] [-m max] [-t type]`
fn parse_find_args(args: &[String]) -> Result<FindArgs> {
    ...
    if has_unsupported_find_flags(args) {
        anyhow::bail!(
            "prltc find does not support compound predicates or actions (e.g. -not, -exec). Use `find` directly."
        );
    }
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            flag if flag.starts_with('-') => {
                eprintln!("prltc find: unknown flag '{}', ignored", flag);
            }
            _ => {}
        }
        i += 1;
...
/// Parse PRLTC syntax: `find <pattern> [path] [-m max] [-t type]`
fn parse_prltc_find_args(args: &[String]) -> Result<FindArgs> {
...
pub fn run(
    ...
    let prltc_output = format!("{}F {}D + {}", total_files, dirs_count, ext_line);
    timer.track(
        &format!("find {} -name '{}'", path, effective_pattern),
        "prltc find",
        &raw_output,
...
mod tests {
    ...
    // --- parse_find_args: PRLTC syntax ---

    #[test]
    fn parse_prltc_syntax_pattern_only() {
    ...
    #[test]
    fn parse_prltc_syntax_pattern_and_path() {
    ...
    #[test]
    fn parse_prltc_syntax_with_flags() {
    ...
    #[test]
    fn run_from_args_prltc_syntax() {
        // Simulates: prltc find *.rs src
