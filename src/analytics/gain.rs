...
//! Shows users how many tokens PRLTC has saved them over time.
...
pub fn run(
    ...
    if summary.total_commands == 0 {
        ...
        println!("Run some prltc commands to start tracking savings.");
        return Ok(());
    }
    ...
    if !daily && !weekly && !monthly && !all {
        ...
        let title = if project_scope.is_some() {
            "PRLTC Token Savings (Project Scope)"
        } else {
            "PRLTC Token Savings (Global Scope)"
        };
        ...
        if history {
            ...
            if !recent.is_empty() {
                ...
                for rec in recent {
                    ...
                    let cmd_short = if rec.prltc_cmd.len() > 25 {
                        format!("{}...", &rec.prltc_cmd[..22])
                    } else {
                        rec.prltc_cmd.clone()
                    };
...
fn show_failures(tracker: &Tracker) -> Result<()> {
    ...
    println!("{}", styled("PRLTC Parse Failures", true));
    println!("{}", "═".repeat(60));
    println!();
