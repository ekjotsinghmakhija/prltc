...
Owns: `prltc gain` (savings dashboard), `prltc cc-economics` (cost reduction), `prltc session` (adoption analysis), and Claude Code usage data parsing.
...
These modules read from the SQLite tracking database to produce dashboards, spending estimates, and session-level adoption reports that help users understand the value PRLTC provides.
...
| File | Responsibility |
|------|---------------|
| gain.rs | `prltc gain` command -- token savings dashboard with ASCII graphs, per-command history, quota consumption estimates, and cumulative savings over time; the primary user-facing analytics view |
| cc_economics.rs | `prltc cc-economics` command -- combines Claude Code API spending data with PRLTC savings to show net cost reduction; correlates token savings with dollar amounts |
...
| session_cmd.rs | `prltc session` command -- per-session PRLTC adoption analysis; shows which commands in a coding session were routed through PRLTC vs executed raw, highlighting missed savings opportunities |
