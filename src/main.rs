...
enum Commands {
    ...
    Find {
        /// All find arguments (supports both PRLTC and native find syntax)
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
    },
    ...
    /// Initialize prltc instructions for assistant CLI usage
    Init {
        ...
        /// Remove PRLTC artifacts for the selected assistant mode
        #[arg(long)]
        uninstall: bool,
    ...
    /// Discover missed PRLTC savings from Claude Code history
    Discover {
    ...
    /// Show hook rewrite audit metrics (requires PRLTC_HOOK_AUDIT=1)
    ...
    /// Rewrite a raw command to its PRLTC equivalent (single source of truth for hooks)
    ///
    /// Exits 0 and prints the rewritten command if supported.
    /// Exits 1 with no output if the command has no PRLTC equivalent.
    ///
    ...
    Rewrite {
        ...
        /// Accepts multiple args: `prltc rewrite ls -al` is equivalent to `prltc rewrite "ls -al"`
        #[arg(trailing_var_arg = true, allow_hyphen_values = true)]
        args: Vec<String>,
    },
...
fn run_fallback(parse_error: clap::Error) -> Result<i32> {
    ...
    // TOML filter lookup — bypass with PRLTC_NO_TOML=1
    ...
    let toml_match = if std::env::var("PRLTC_NO_TOML").ok().as_deref() == Some("1") {
        None
    } else {
...
fn run_cli() -> Result<i32> {
    ...
    let code = match cli.command {
        ...
        Commands::Read {
            ...
            for file in &files {
                let result = if file == Path::new("-") {
                    if stdin_seen {
                        eprintln!("prltc: warning: stdin specified more than once");
                        continue;
                    }
                    stdin_seen = true;
        ...
        Commands::Proxy { args } => {
            ...
            // e.g. prltc proxy 'head -50 file.php' → cmd=head, args=["-50", "file.php"]
            // e.g. prltc proxy 'git log --format="%H %s"' → cmd=git, args=["log", "--format=%H %s"]
...
mod tests {
    ...
    fn test_git_global_options_parsing() {
        let cli =
            Cli::try_parse_from(["prltc", "git", "--no-pager", "--no-optional-locks", "status"])
                .unwrap();
        match cli.command {
            Commands::Git {
                no_pager,
    ...
    fn test_try_parse_git_with_dash_c_succeeds() {
        let result = Cli::try_parse_from(["prltc", "git", "-C", "/path", "status"]);
        assert!(
            result.is_ok(),
    ...
    fn test_rewrite_clap_multi_args() {
        // This is the bug KuSh reported: `prltc rewrite ls -al` failed because
        ...
        let cases = vec![
            vec!["prltc", "rewrite", "ls", "-al"],
            vec!["prltc", "rewrite", "git", "status"],
            vec!["prltc", "rewrite", "npm", "exec"],
            vec!["prltc", "rewrite", "cargo", "test"],
            vec!["prltc", "rewrite", "du", "-sh", "."],
            vec!["prltc", "rewrite", "head", "-50", "file.txt"],
        ];
        for args in &cases {
            let result = Cli::try_parse_from(args.iter());
            assert!(
                result.is_ok(),
                "prltc rewrite {:?} should parse (was failing before trailing_var_arg fix)",
                &args[2..]
            );
            if let Ok(cli) = result {
                match cli.command {
    ...
    fn test_rewrite_clap_quoted_single_arg() {
        // Quoted form: `prltc rewrite "git status"` — single arg containing spaces
        let result = Cli::try_parse_from(["prltc", "rewrite", "git status"]);
        assert!(result.is_ok());
        if let Ok(cli) = result {
            match cli.command {
                Commands::Rewrite { ref args } => {
