...
pub struct PeriodEconomics {
    ...
    // prltc metrics
    pub prltc_commands: Option<usize>,
    pub prltc_saved_tokens: Option<usize>,
    pub prltc_savings_pct: Option<f64>,
...
impl PeriodEconomics {
    fn new(label: &str) -> Self {
        Self {
            ...
            prltc_commands: None,
            prltc_saved_tokens: None,
            prltc_savings_pct: None,
    ...
    fn set_prltc_from_day(&mut self, stats: &DayStats) {
        self.prltc_commands = Some(stats.commands);
        self.prltc_saved_tokens = Some(stats.saved_tokens);
        self.prltc_savings_pct = Some(stats.savings_pct);
    }

    fn set_prltc_from_week(&mut self, stats: &WeekStats) {
        self.prltc_commands = Some(stats.commands);
        self.prltc_saved_tokens = Some(stats.saved_tokens);
        self.prltc_savings_pct = Some(stats.savings_pct);
    }

    fn set_prltc_from_month(&mut self, stats: &MonthStats) {
        self.prltc_commands = Some(stats.commands);
        self.prltc_saved_tokens = Some(stats.saved_tokens);
        self.prltc_savings_pct = Some(if stats.input_tokens + stats.output_tokens > 0 {
    ...
    fn compute_weighted_metrics(&mut self) {
        ...
        if let (Some(cost), Some(saved)) = (self.cc_cost, self.prltc_saved_tokens) {
...
struct Totals {
    ...
    prltc_commands: usize,
    prltc_saved_tokens: usize,
    prltc_avg_savings_pct: f64,
...
fn merge_daily(cc: Option<Vec<CcusagePeriod>>, prltc: Vec<DayStats>) -> Vec<PeriodEconomics> {
...
fn merge_weekly(cc: Option<Vec<CcusagePeriod>>, prltc: Vec<WeekStats>) -> Vec<PeriodEconomics> {
    let mut map: HashMap<String, PeriodEconomics> = HashMap::new();
    ...
    // Merge prltc data (week_start = legacy Saturday "2026-01-18")
...
/// Convert Saturday week_start (legacy prltc) to ISO Monday
...
fn convert_saturday_to_monday(saturday: &str) -> Option<String> {
    ...
    // prltc uses Saturday as week start, ISO uses Monday
...
fn compute_totals(periods: &[PeriodEconomics]) -> Totals {
    let mut totals = Totals {
        ...
        prltc_commands: 0,
        prltc_saved_tokens: 0,
        prltc_avg_savings_pct: 0.0,
        weighted_input_cpt: None,
        savings_weighted: None,
        blended_cpt: None,
        active_cpt: None,
    ...
    for p in periods {
        ...
        if let Some(cmds) = p.prltc_commands {
            totals.prltc_commands += cmds;
        }
        if let Some(saved) = p.prltc_saved_tokens {
            totals.prltc_saved_tokens += saved;
        }
        if let Some(pct) = p.prltc_savings_pct {
            pct_sum += pct;
            pct_count += 1;
        }
    }
    ...
    if pct_count > 0 {
        totals.prltc_avg_savings_pct = pct_sum / pct_count as f64;
    }
    ...
    if weighted_units > 0.0 {
        ...
        totals.savings_weighted = Some(totals.prltc_saved_tokens as f64 * input_cpt);
    }
    ...
    if totals.cc_total_tokens > 0 {
        ...
        totals.savings_blended = Some(totals.prltc_saved_tokens as f64 * totals.blended_cpt.unwrap());
    }
    if totals.cc_active_tokens > 0 {
        totals.active_cpt = Some(totals.cc_cost / totals.cc_active_tokens as f64);
        totals.savings_active = Some(totals.prltc_saved_tokens as f64 * totals.active_cpt.unwrap());
    }
...
fn display_summary(tracker: &Tracker, verbose: u8) -> Result<()> {
    ...
    if periods.is_empty() {
        println!("No data available. Run some prltc commands to start tracking.");
        return Ok(());
    }
    ...
    println!("  PRLTC commands:                 {}", totals.prltc_commands);
    println!(
        "  Tokens saved:                 {}",
        format_tokens(totals.prltc_saved_tokens)
    );
    println!();
    ...
    println!("  PRLTC compresses CLI outputs before they enter Claude's context.");
...
fn print_period_table(periods: &[PeriodEconomics], verbose: u8) {
    ...
    if verbose > 0 {
        ...
        println!(
            ...
            "Period", "Spent", "Saved", "Savings", "Active$", "Blended$", "PRLTC Cmds"
        );
        println!(
            "{:-<12} {:-<10} {:-<10} {:-<10} {:-<10} {:-<12} {:-<12}",
            "", "", "", "", "", "", ""
        ...
        println!(
            "{:<12} {:>10} {:>10} {:>10} {:>12}",
            "Period", "Spent", "Saved", "Savings", "PRLTC Cmds"
        );
        println!(
            "{:-<12} {:-<10} {:-<10} {:-<10} {:-<12}",
            "", "", "", "", ""
...
fn export_csv(
    ...
    println!("period,spent,input_tokens,output_tokens,cache_create,cache_read,active_tokens,total_tokens,saved_tokens,weighted_savings,active_savings,blended_savings,prltc_commands");
...
fn print_csv_row(p: &PeriodEconomics) {
    ...
    let blended_savings = p
        ...
        .map(|s| format!("{:.4}", s))
        .unwrap_or_default();
    let cmds = p.prltc_commands.map(|c| c.to_string()).unwrap_or_default();
...
mod tests {
    ...
    fn test_period_economics_new() {
        ...
        assert!(p.prltc_commands.is_none());
    }
    ...
    fn test_merge_monthly_both_present() {
        ...
        assert_eq!(merged[0].prltc_commands, Some(10));
    }
    ...
    fn test_merge_monthly_only_ccusage() {
        ...
        assert_eq!(merged.len(), 1);
        assert_eq!(merged[0].cc_cost, Some(12.34));
        assert!(merged[0].prltc_commands.is_none());
    }

    #[test]
    fn test_merge_monthly_only_prltc() {
        let prltc = vec![MonthStats {
            month: "2026-01".to_string(),
            commands: 10,
            input_tokens: 800,
    ...
    fn test_compute_weighted_input_cpt() {
        ...
        p.prltc_saved_tokens = Some(10_000);
    ...
    fn test_compute_weighted_metrics_zero_tokens() {
        ...
        p.prltc_saved_tokens = Some(5000);
    ...
    fn test_compute_weighted_metrics_no_cache() {
        ...
        p.cc_cache_create_tokens = Some(0);
        p.cc_cache_read_tokens = Some(0);
        p.prltc_saved_tokens = Some(3000);
    ...
    fn test_compute_totals() {
        let periods = vec![
            PeriodEconomics {
                ...
                prltc_commands: Some(5),
                prltc_saved_tokens: Some(2000),
                prltc_savings_pct: Some(50.0),
                weighted_input_cpt: None,
                savings_weighted: None,
                blended_cpt: None,
                active_cpt: None,
            ...
            PeriodEconomics {
                ...
                prltc_commands: Some(10),
                prltc_saved_tokens: Some(3000),
                prltc_savings_pct: Some(60.0),
                weighted_input_cpt: None,
                savings_weighted: None,
                blended_cpt: None,
                active_cpt: None,
        ...
        assert_eq!(totals.prltc_commands, 15);
        assert_eq!(totals.prltc_saved_tokens, 5000);
        assert_eq!(totals.prltc_avg_savings_pct, 55.0);
