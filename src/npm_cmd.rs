...
pub fn run(args: &[String], verbose: u8, skip_env: bool) -> Result<()> {
    ...
    let effective_args = if is_run_explicit {
        // "prltc npm run build" → "npm run build"
        cmd.arg("run");
        &args[1..]
    } else if is_npm_subcommand {
        // "prltc npm install express" → "npm install express"
        args
    } else {
        // "prltc npm build" → "npm run build" (assume script name)
        cmd.arg("run");
        args
    };
