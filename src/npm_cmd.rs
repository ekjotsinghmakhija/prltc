...
pub fn run(args: &[String], verbose: u8, skip_env: bool) -> Result<()> {
    ...
    // Strip leading "run" to avoid doubling (prltc npm run build → npm run build, not npm run run build)
    ...
    timer.track(
        ...
        &format!("prltc npm run {}", effective_args.join(" ")),
        &raw,
        &filtered,
    );
...
"#;
    ...
    fn test_strip_leading_run_from_args() {
        // When user runs `prltc npm run build`, args = ["run", "build"]
        ...
        // When user runs `prltc npm build`, args = ["build"]
        // No stripping needed
        ...
        // When user runs `prltc npm run`, args = ["run"]
