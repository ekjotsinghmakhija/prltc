...
fn run_aws_filtered(
    ...
    let prltc_label = format!("prltc {}", cmd_label);
    ...
    if !status.success() {
        ...
        timer.track(&cmd_label, &prltc_label, &raw, &stderr);
        return Ok(exit_code);
    }

    let result = filter_fn(&stdout).unwrap_or_else(|| {
        eprintln!("prltc: filter warning: aws filter returned None, passing through raw output");
    ...
    timer.track(&cmd_label, &prltc_label, &raw, &result.text);
    Ok(0)
}
...
fn run_s3_ls(extra_args: &[String], verbose: u8) -> Result<i32> {
    ...
    if !output.status.success() {
        ...
        timer.track("aws s3 ls", "prltc aws s3 ls", &raw, &stderr);
        return Ok(exit_code);
    }
    ...
    timer.track("aws s3 ls", "prltc aws s3 ls", &raw, &result.text);
    Ok(0)
}
...
fn run_s3_transfer(operation: &str, extra_args: &[String], verbose: u8) -> Result<i32> {
    ...
    let prltc_label = format!("prltc aws s3 {}", operation);
