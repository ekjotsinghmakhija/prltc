...
**Command execution and output filtering** — this is the core value PRLTC delivers. Every module here calls an external CLI tool (`Command::new("some_tool")`), transforms its stdout/stderr to reduce token consumption, and records savings via `core/tracking`.
...
pub fn run(args: MyArgs, verbose: u8) -> Result<()> {
    ...
    timer.track("mycmd args", "prltc mycmd args", &raw, &filtered);
