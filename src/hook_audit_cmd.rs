...
/// Default log file location (aligned with hook's $HOME/.local/share/prltc/).
fn default_log_path() -> PathBuf {
    if let Ok(dir) = std::env::var("PRLTC_AUDIT_DIR") {
        PathBuf::from(dir).join("hook-audit.log")
    } else {
        let home = std::env::var("HOME").unwrap_or_else(|_| "/tmp".to_string());
        PathBuf::from(home)
            .join(".local/share/prltc")
            .join("hook-audit.log")
    }
}
...
pub fn run(since_days: u64, verbose: u8) -> Result<()> {
    ...
    if !log_path.exists() {
        ...
        println!("Enable audit mode: export PRLTC_HOOK_AUDIT=1 in your shell, then use Claude Code.");
        return Ok(());
    }
...
mod tests {
    ...
    fn test_parse_line_rewrite() {
        let line = "2026-02-16T14:30:01Z | rewrite | git status | prltc git status";
        let entry = parse_line(line).unwrap();
        assert_eq!(entry.action, "rewrite");
        assert_eq!(entry.original_cmd, "git status");
        assert_eq!(entry._rewritten_cmd, "prltc git status");
    }
    ...
    fn test_token_savings() {
        // Simulate what prltc hook-audit would output vs raw log dump
        let raw_log = r#"2026-02-16T14:30:01Z | rewrite | git status | prltc git status
2026-02-16T14:30:02Z | skip:no_match | echo hello | -
2026-02-16T14:30:03Z | rewrite | cargo test | prltc cargo test
2026-02-16T14:30:04Z | skip:already_prltc | prltc git log | -
2026-02-16T14:30:05Z | rewrite | git log --oneline -10 | prltc git log --oneline -10
2026-02-16T14:30:06Z | rewrite | gh pr view 42 | prltc gh pr view 42
2026-02-16T14:30:07Z | skip:no_match | mkdir -p foo | -
2026-02-16T14:30:08Z | rewrite | cargo clippy --all-targets | prltc cargo clippy --all-targets"#;
