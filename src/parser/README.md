...
This ensures PRLTC **never returns false data silently** while maintaining maximum token efficiency.
...
3. **Degradation warnings**: print `[PRLTC:DEGRADED]` in verbose mode, `[PRLTC:PASSTHROUGH]` on full fallback
...
```
[PRLTC:DEGRADED] vitest parser: JSON parse failed at line 42, using regex fallback
[PRLTC:PASSTHROUGH] playwright parser: Pattern mismatch, showing truncated output
```
