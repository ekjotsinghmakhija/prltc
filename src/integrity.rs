...
//!
//! PRLTC installs a PreToolUse hook (`prltc-rewrite.sh`) that auto-approves
...
//! - Manual verification via `prltc verify`
//!
...
const HASH_FILENAME: &str = ".prltc-hook.sha256";
...
pub enum IntegrityStatus {
    ...
    Verified,
    /// Hash mismatch — hook has been modified outside of `prltc init`
    Tampered { expected: String, actual: String },
    /// Hook exists but no stored hash (installed before integrity checks)
    NoBaseline,
    /// Neither hook nor hash file exist (PRLTC not installed)
    NotInstalled,
...
/// Run integrity check and print results (for `prltc verify` subcommand)
pub fn run_verify(verbose: u8) -> Result<()> {
    ...
    match verify_hook_at(&hook_path)? {
        ...
        IntegrityStatus::Tampered { expected, actual } => {
            ...
            eprintln!();
            eprintln!("  The hook file has been modified outside of `prltc init`.");
            eprintln!("  This could indicate tampering or a manual edit.");
            eprintln!();
            eprintln!("  To restore: prltc init -g --auto-patch");
        ...
        IntegrityStatus::NoBaseline => {
            ...
            println!("      Run `prltc init -g` to establish baseline.");
        }
        IntegrityStatus::NotInstalled => {
            println!("SKIP  PRLTC hook not installed");
            println!("      Run `prltc init -g` to install.");
        }
        IntegrityStatus::OrphanedHash => {
            eprintln!("WARN  hash file exists but hook is missing");
            eprintln!("      Run `prltc init -g` to reinstall.");
        }
    }
...
///
/// Can be bypassed with `PRLTC_SKIP_INTEGRITY=1` for emergencies.
pub fn runtime_check() -> Result<()> {
    // Allow emergency bypass
    if std::env::var("PRLTC_SKIP_INTEGRITY").unwrap_or_default() == "1" {
        return Ok(());
    }
    ...
    match verify_hook()? {
        ...
        IntegrityStatus::Tampered { expected, actual } => {
            ...
            eprintln!();
            eprintln!("  To restore:  prltc init -g --auto-patch");
            eprintln!("  To inspect:  prltc verify");
            eprintln!("  To override: PRLTC_SKIP_INTEGRITY=1 prltc <command>");
            std::process::exit(1);
        }
        IntegrityStatus::OrphanedHash => {
            eprintln!("prltc: warning: hash file exists but hook is missing");
            eprintln!("  Run `prltc init -g` to reinstall.");
...
mod tests {
    ...
    fn test_store_and_verify_ok() {
        let temp = TempDir::new().unwrap();
        let hook = temp.path().join("prltc-rewrite.sh");
    ...
    fn test_verify_orphaned_hash() {
        let temp = TempDir::new().unwrap();
        let hook = temp.path().join("prltc-rewrite.sh");
        let hash_file = temp.path().join(".prltc-hook.sha256");

        // Create hash but no hook
        fs::write(
            &hash_file,
            "abc123def456abc123def456abc123def456abc123def456abc123def456abc12345  prltc-rewrite.sh\n",
        )
        .unwrap();
    ...
    fn test_store_hash_creates_sha256sum_format() {
        ...
        // Format: "<64 hex chars>  prltc-rewrite.sh\n"
        assert!(content.ends_with("  prltc-rewrite.sh\n"));
    ...
    fn test_invalid_hash_file_rejected() {
        ...
        fs::write(&hook, "test").unwrap();
        fs::write(&hash_file, "not-a-valid-hash  prltc-rewrite.sh\n").unwrap();
    ...
    fn test_hash_format_compatible_with_sha256sum() {
        ...
        assert_eq!(parts[1], "prltc-rewrite.sh");
    }
}
