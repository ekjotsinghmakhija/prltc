            ...
            for part in parts {
                ...
                match classify_command(part) {
                    Classification::Supported {
                        ...
                        let bucket = supported_map.entry(prltc_equivalent).or_insert_with(|| {
                            SupportedBucket {
                                prltc_equivalent,
                                category,
                                count: 0,
                    ...
                    Classification::Ignored => {
                        // Check if it starts with "prltc "
                        if part.trim().starts_with("prltc ") {
                            already_prltc += 1;
                        }
                        // Otherwise just skip
                    }
