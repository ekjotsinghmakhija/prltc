...
/// Check if args contain --json flag (user wants specific JSON fields, not PRLTC filtering)
...
fn view_pr(args: &[String], _verbose: u8, ultra_compact: bool) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        timer.track(
            ...
            &format!("prltc gh pr view {}", pr_number),
            &stderr,
            &stderr,
        );
        eprintln!("{}", stderr.trim());
...
fn pr_checks(args: &[String], _verbose: u8, _ultra_compact: bool) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        timer.track(
            ...
            &format!("prltc gh pr checks {}", pr_number),
            &stderr,
            &stderr,
        );
        eprintln!("{}", stderr.trim());
...
fn view_issue(args: &[String], _verbose: u8) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        timer.track(
            ...
            &format!("prltc gh issue view {}", issue_number),
            &stderr,
            &stderr,
        );
        eprintln!("{}", stderr.trim());
...
fn view_run(args: &[String], _verbose: u8) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        timer.track(
            ...
            &format!("prltc gh run view {}", run_id),
            &stderr,
            &stderr,
        );
        eprintln!("{}", stderr.trim());
...
fn run_api(args: &[String], _verbose: u8) -> Result<()> {
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr).to_string();
        timer.track("gh api", "prltc gh api", &stderr, &stderr);
        eprintln!("{}", stderr.trim());
        std::process::exit(output.status.code().unwrap_or(1));
    }
    ...
    timer.track("gh api", "prltc gh api", &raw, &filtered);
    Ok(())
}
...
mod tests {
    ...
    fn test_extract_identifier_with_repo_flag_after() {
        // gh issue view 185 -R ekjotsinghmakhija/prltc
        let args: Vec<String> = vec!["185".into(), "-R".into(), "ekjotsinghmakhija/prltc".into()];
        let (id, extra) = extract_identifier_and_extra_args(&args).unwrap();
        assert_eq!(id, "185");
        assert_eq!(extra, vec!["-R", "ekjotsinghmakhija/prltc"]);
    }

    #[test]
    fn test_extract_identifier_with_repo_flag_before() {
        // gh issue view -R ekjotsinghmakhija/prltc 185
        let args: Vec<String> = vec!["-R".into(), "ekjotsinghmakhija/prltc".into(), "185".into()];
        let (id, extra) = extract_identifier_and_extra_args(&args).unwrap();
        assert_eq!(id, "185");
        assert_eq!(extra, vec!["-R", "ekjotsinghmakhija/prltc"]);
    }
    ...
    fn test_extract_identifier_only_flags() {
        ...
        let args: Vec<String> = vec!["-R".into(), "ekjotsinghmakhija/prltc".into()];
        assert!(extract_identifier_and_extra_args(&args).is_none());
    }
...
[![CI](https://img.shields.io/github/actions/workflow/status/ekjotsinghmakhija/prltc/ci.yml)](https://github.com/ekjotsinghmakhija/prltc/actions)
[![Coverage](https://img.shields.io/codecov/c/github/ekjotsinghmakhija/prltc)](https://codecov.io/gh/ekjotsinghmakhija/prltc)
