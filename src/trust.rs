...
//! Trust boundary for project-local TOML filters (SA-2025-PRLTC-002).
//!
//! `.prltc/filters.toml` is loaded from CWD with highest priority. An attacker
...
//! - `prltc trust` stores the SHA-256 hash after user review
...
pub fn check_trust(filter_path: &Path) -> Result<TrustStatus> {
    ...
    if std::env::var("PRLTC_TRUST_PROJECT_FILTERS").as_deref() == Ok("1") {
        ...
        eprintln!(
            "[prltc] WARNING: PRLTC_TRUST_PROJECT_FILTERS=1 ignored (CI environment not detected)"
        );
    }
    ...
    let store = match read_store() {
        ...
        Err(e) => {
            eprintln!(
                "[prltc] WARNING: trust store unreadable ({}), treating all filters as untrusted",
                e
            );
            TrustStore::default()
        }
...
/// Run `prltc trust` — review and trust project-local filters.
pub fn run_trust(list: bool) -> Result<()> {
    ...
    let filter_path = Path::new(".prltc/filters.toml");
    if !filter_path.exists() {
        anyhow::bail!("No .prltc/filters.toml found in current directory");
    }

    // Read ONCE to prevent TOCTOU: display + hash from same buffer
    let content_bytes = std::fs::read(filter_path).context("Failed to read .prltc/filters.toml")?;
    let content = String::from_utf8_lossy(&content_bytes);

    println!("=== .prltc/filters.toml ===");
    println!("{}", content);
    println!("=========================");
    println!();
    ...
    println!();
    println!(
        "Trusted .prltc/filters.toml (sha256:{})",
...
/// Run `prltc untrust` — revoke trust for project-local filters.
pub fn run_untrust() -> Result<()> {
    ...
    if removed {
        println!("Trust revoked for .prltc/filters.toml");
...
mod tests {
    ...
    fn test_env_override_with_ci() {
        ...
        #[allow(deprecated)]
        std::env::set_var("PRLTC_TRUST_PROJECT_FILTERS", "1");
        #[allow(deprecated)]
        std::env::set_var("CI", "true");
        let status = check_trust(&filter).unwrap();
        #[allow(deprecated)]
        std::env::remove_var("PRLTC_TRUST_PROJECT_FILTERS");
        #[allow(deprecated)]
