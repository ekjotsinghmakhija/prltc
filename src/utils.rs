...
//!
//! Provides common helpers used across prltc commands:
...
/// # Examples
/// ```no_run
/// use prltc::utils::execute_command;
...
/// # Examples
/// ```
/// use prltc::utils::join_with_overflow;
...
/// # Examples
/// ```
/// use prltc::utils::truncate_iso_date;
...
/// # Examples
/// ```
/// use prltc::utils::ok_confirmation;
        ...
        None => {
            #[cfg(unix)]
            {
                ...
                if let Some(sig) = output.status.signal() {
                    eprintln!("[prltc] {}: process terminated by signal {}", label, sig);
                    return 128 + sig;
                }
            }
            eprintln!("[prltc] {}: process terminated by signal", label);
            1
        }
    }
}
...
pub fn fallback_tail(output: &str, label: &str, n: usize) -> String {
    eprintln!(
        "[prltc] {}: output format not recognized, showing last {} lines",
        label, n
    );
...
/// # Examples
/// ```no_run
/// use prltc::utils::detect_package_manager;
...
pub fn resolved_command(name: &str) -> Command {
    match resolve_binary(name) {
        ...
        Err(e) => {
            ...
            eprintln!(
                "prltc: Failed to resolve '{}' via PATH, falling back to direct exec: {}",
                name, e
            );
...
mod tests {
    ...
    fn test_detect_package_manager_default() {
        // In the test environment (prltc repo), there's no JS lockfile
