...
enum HookFormat {
    ...
    /// Non-bash tool, already uses prltc, or unknown format — pass through silently.
    PassThrough,
}
...
pub fn run_copilot() -> Result<()> {
    ...
    let v: Value = match serde_json::from_str(input) {
        Ok(v) => v,
        Err(e) => {
            eprintln!("[prltc hook] Failed to parse JSON input: {e}");
            return Ok(());
        }
    };
...
fn handle_copilot_cli(cmd: &str) -> Result<()> {
    ...
    let output = json!({
        ...
        "permissionDecisionReason": format!(
            "Token savings: use `{}` instead (prltc saves 60-90% tokens)",
            rewritten
        )
    });
    println!("{output}");
...
/// Reads JSON from stdin, rewrites shell commands to prltc equivalents,
...
pub fn run_gemini() -> Result<()> {
    ...
    if check_command(cmd) == PermissionVerdict::Deny {
        println!(r#"{{"decision":"deny","reason":"Blocked by PRLTC permission rule"}}"#);
        return Ok(());
    }
...
mod tests {
    ...
    #[test]
    fn test_get_rewritten_already_prltc() {
        assert!(get_rewritten("prltc git status").is_none());
    }
    ...
    fn test_gemini_hook_uses_rewrite_command() {
        ...
        // Already prltc → returned as-is (idempotent)
        assert_eq!(
    ...
    fn test_gemini_hook_env_prefix_preserved() {
        assert_eq!(
            ...
            Some("RUST_LOG=debug prltc cargo test".into())
        );
    }
}
