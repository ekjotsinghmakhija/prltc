...
pub fn run_format(args: &[String], verbose: u8) -> Result<()> {
    ...
    timer.track(
        ...
        &format!("prltc dotnet format {}", args.join(" ")),
        &raw,
        &filtered,
    );
...
pub fn run_passthrough(args: &[OsString], verbose: u8) -> Result<()> {
    ...
    timer.track(
        ...
        &format!("prltc dotnet {}", subcommand),
        &raw,
        &raw,
    );
...
fn run_dotnet_with_binlog(subcommand: &str, args: &[String], verbose: u8) -> Result<()> {
    ...
    timer.track(
        ...
        &format!("prltc dotnet {} {}", subcommand, args.join(" ")),
        &raw,
        &output_to_print,
    );
...
fn build_trx_results_dir() -> PathBuf {
    std::env::temp_dir().join(format!("prltc_dotnet_testresults_{}", unique_temp_suffix()))
}
...
fn build_format_report_path() -> PathBuf {
    std::env::temp_dir().join(format!("prltc_dotnet_format_{}.json", unique_temp_suffix()))
}
