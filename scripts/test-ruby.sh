#!/usr/bin/env bash
#
# PRLTC Smoke Tests — Ruby (RSpec, RuboCop, Minitest, Bundle)
# Creates a minimal Rails app, exercises all Ruby PRLTC filters, then cleans up.
# Usage: bash scripts/test-ruby.sh
#
# Prerequisites: prltc (installed), ruby, bundler, rails gem
...
printf "${BOLD}PRLTC Smoke Tests — Ruby (RSpec, RuboCop, Minitest, Bundle)${NC}\n"
printf "Binary: %s (%s)\n" "$PRLTC" "$(prltc --version)"
printf "Ruby: %s\n" "$(ruby --version)"
printf "Rails: %s\n" "$(rails --version)"
printf "Bundler: %s\n" "$(bundle --version)"
...
TMPDIR=$(mktemp -d /tmp/prltc-ruby-smoke-XXXXXX)
...
rails new prltc_smoke_app --minimal --skip-git --skip-docker --quiet 2>&1 | tail -1 || true
cd prltc_smoke_app
...
assert_output "prltc rspec (with failure)" \
    "failed" \
    prltc rspec

assert_output "prltc rspec spec/models/post_spec.rb (pass)" \
    "RSpec.*passed" \
    prltc rspec spec/models/post_spec.rb

assert_output "prltc rspec spec/models/post_fail_spec.rb (fail)" \
    "failed\|❌" \
    prltc rspec spec/models/post_fail_spec.rb

# ── 2. RuboCop ───────────────────────────────────────

section "RuboCop"

assert_output "prltc rubocop (with offenses)" \
    "offense" \
    prltc rubocop

assert_output "prltc rubocop app/ (with offenses)" \
...
assert_output "prltc rake test (with failure)" \
    "failure\|error\|FAIL" \
    prltc rake test

assert_output "prltc rake test single passing file" \
    "ok rake test\|0 failures" \
    prltc rake test TEST=test/models/post_pass_test.rb

assert_exit_nonzero "prltc rake test single failing file" \
    "failure\|FAIL" \
    prltc rake test test/models/post_fail_test.rb

# ── 4. Bundle install ────────────────────────────────

section "Bundle install"

assert_output "prltc bundle install (idempotent)" \
    "bundle\|ok\|complete\|install" \
    prltc bundle install

# ── 5. Exit code preservation ────────────────────────

section "Exit code preservation"

assert_exit_nonzero "prltc rspec exits non-zero on failure" \
    "failed\|failure" \
    prltc rspec spec/models/post_fail_spec.rb

assert_exit_nonzero "prltc rubocop exits non-zero on offenses" \
    "offense" \
    prltc rubocop app/models/rubocop_bait.rb

assert_exit_nonzero "prltc rake test exits non-zero on failure" \
    "failure\|FAIL" \
    prltc rake test test/models/post_fail_test.rb
...
assert_output "bundle exec rspec spec/models/post_spec.rb" \
    "passed\|example" \
    prltc bundle exec rspec spec/models/post_spec.rb

assert_output "bundle exec rubocop app/" \
    "offense" \
    prltc bundle exec rubocop app/
...
assert_output "prltc rubocop -A (autocorrect)" \
    "autocorrected\|rubocop\|ok\|offense\|inspected" \
    prltc rubocop -A app/models/rubocop_bait_ac.rb
...
assert_output "prltc rspec with pending example" \
    "pending" \
    prltc rspec spec/models/post_pending_spec.rb

# ── 9. RSpec text fallback ───────────────────────────

section "RSpec text fallback"

assert_output "prltc rspec --format documentation (text path)" \
    "valid\|example\|post" \
    prltc rspec --format documentation spec/models/post_spec.rb

# ── 10. RSpec empty suite ────────────────────────────

section "RSpec empty suite"

assert_output "prltc rspec nonexistent tag" \
    "0 examples\|No examples" \
    prltc rspec --tag nonexistent spec/models/post_spec.rb
...
prltc_len=$( (prltc rspec spec/models/post_spec.rb 2>&1 || true) | wc -c | tr -d ' ')
if [[ "$prltc_len" -lt "$raw_len" ]]; then
    PASS=$((PASS + 1))
    printf "  ${GREEN}PASS${NC}  rspec: prltc (%s bytes) < raw (%s bytes)\n" "$prltc_len" "$raw_len"
else
    FAIL=$((FAIL + 1))
    FAILURES+=("token savings: rspec")
    printf "  ${RED}FAIL${NC}  rspec: prltc (%s bytes) >= raw (%s bytes)\n" "$prltc_len" "$raw_len"
fi

# rubocop (exits non-zero on offenses, so || true)
raw_len=$( (bundle exec rubocop app/ 2>&1 || true) | wc -c | tr -d ' ')
prltc_len=$( (prltc rubocop app/ 2>&1 || true) | wc -c | tr -d ' ')
if [[ "$prltc_len" -lt "$raw_len" ]]; then
    PASS=$((PASS + 1))
    printf "  ${GREEN}PASS${NC}  rubocop: prltc (%s bytes) < raw (%s bytes)\n" "$prltc_len" "$raw_len"
else
    FAIL=$((FAIL + 1))
    FAILURES+=("token savings: rubocop")
    printf "  ${RED}FAIL${NC}  rubocop: prltc (%s bytes) >= raw (%s bytes)\n" "$prltc_len" "$raw_len"
fi

# rake test (passing file)
raw_len=$( (bundle exec rake test TEST=test/models/post_pass_test.rb 2>&1 || true) | wc -c | tr -d ' ')
prltc_len=$( (prltc rake test test/models/post_pass_test.rb 2>&1 || true) | wc -c | tr -d ' ')
if [[ "$prltc_len" -lt "$raw_len" ]]; then
    PASS=$((PASS + 1))
    printf "  ${GREEN}PASS${NC}  rake test: prltc (%s bytes) < raw (%s bytes)\n" "$prltc_len" "$raw_len"
else
    FAIL=$((FAIL + 1))
    FAILURES+=("token savings: rake test")
    printf "  ${RED}FAIL${NC}  rake test: prltc (%s bytes) >= raw (%s bytes)\n" "$prltc_len" "$raw_len"
fi

# bundle install (idempotent)
raw_len=$( (bundle install 2>&1 || true) | wc -c | tr -d ' ')
prltc_len=$( (prltc bundle install 2>&1 || true) | wc -c | tr -d ' ')
if [[ "$prltc_len" -lt "$raw_len" ]]; then
    PASS=$((PASS + 1))
    printf "  ${GREEN}PASS${NC}  bundle install: prltc (%s bytes) < raw (%s bytes)\n" "$prltc_len" "$raw_len"
else
    FAIL=$((FAIL + 1))
    FAILURES+=("token savings: bundle install")
    printf "  ${RED}FAIL${NC}  bundle install: prltc (%s bytes) >= raw (%s bytes)\n" "$prltc_len" "$raw_len"
fi
...
assert_output "prltc -v rspec (verbose)" \
    "RSpec\|passed\|Running\|example" \
    prltc -v rspec spec/models/post_spec.rb
