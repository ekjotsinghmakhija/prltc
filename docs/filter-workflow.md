...
This document explains what happens between "I created `src/filters/my-tool.toml`" and "PRLTC filters the output of `my-tool`".
...
flowchart TD
    ...
    subgraph RUNTIME ["⚡ prltc my-tool --verbose"]
        Q["Claude Code hook\nmy-tool ... → prltc my-tool ..."] --> R
        R["TomlFilterRegistry::load()\n① .prltc/filters.toml  (project)\n② ~/.config/prltc/filters.toml  (user)\n③ BUILTIN_TOML  (binary)\n④ passthrough"] --> S
...
| 5 | `prltc verify` | Runs each `[[tests.my-tool]]` entry | Filter logic doesn't match expected output |
...
```mermaid
flowchart LR
    CMD["prltc my-tool args"] --> P1
    P1{"1. .prltc/filters.toml\n(project-local)"}
    P1 -->|"✅ match"| WIN["apply filter"]
    P1 -->|"no match"| P2
    P2{"2. ~/.config/prltc/filters.toml\n(user-global)\n(macOS alt: ~/Library/Application Support/prltc/filters.toml)"}
    P2 -->|"✅ match"| WIN
    P2 -->|"no match"| P3
    P3{"3. BUILTIN_TOML\n(binary)"}
    P3 -->|"✅ match"| WIN
...
```
[prltc] warning: filter 'make' is shadowing a built-in filter
```
