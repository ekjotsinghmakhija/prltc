# PRLTC Token Savings Audit Guide

Complete guide to analyzing your prltc token savings with temporal breakdowns and data exports.

## Overview

The `prltc gain` command provides comprehensive analytics for tracking your token savings across time periods.

**Database Location**: `~/.local/share/prltc/history.db`
...
# Temporal breakdowns
prltc gain --daily          # All days since tracking started
prltc gain --weekly         # Aggregated by week
prltc gain --monthly        # Aggregated by month
prltc gain --all            # Show all breakdowns at once
...
# Combined flags
prltc gain --graph --history --quota    # Classic view with extras
prltc gain --daily --weekly --monthly   # Multiple breakdowns
```
...
**Metrics explained:**
- **Cmds**: Number of prltc commands executed
- **Input**: Estimated tokens from raw command output
- **Output**: Actual tokens after prltc filtering
...
prltc gain --weekly --format csv > reports/week-$(date +%Y-%W).csv

# Compare this week vs last week
prltc gain --weekly | tail -3
```
...
prltc gain --monthly --format json | jq '.monthly[] |
...
# Get CSV data
result = subprocess.run(['prltc', 'gain', '--all', '--format', 'csv'],
...
1. Export CSV: `prltc gain --all --format csv > prltc-data.csv`
2. Open in Excel
...
0 0 * * * prltc gain --all --format json > /var/www/dashboard/prltc-stats.json
...
prltc estimates tokens using `text.len() / 4` (4 characters per token average).
...
Output Tokens   = estimate_tokens(prltc_filtered_output)
...
|---------|----------------|-----------|
| `prltc git status` | 77-93% | Compact stat format |
| `prltc eslint` | 84% | Group by rule |
| `prltc vitest run` | 94-99% | Show failures only |
| `prltc find` | 75% | Tree format |
| `prltc pnpm list` | 70-90% | Compact dependencies |
| `prltc grep` | 70% | Truncate + group |
...
sqlite3 ~/.local/share/prltc/history.db \
  "SELECT timestamp, prltc_cmd, saved_tokens FROM commands
...
```bash
# Backup
cp ~/.local/share/prltc/history.db ~/backups/prltc-history-$(date +%Y%m%d).db

# Restore
cp ~/backups/prltc-history-20260128.db ~/.local/share/prltc/history.db

# Export for migration
sqlite3 ~/.local/share/prltc/history.db .dump > prltc-backup.sql
```
...
# Reset all data
rm ~/.local/share/prltc/history.db
# Next prltc command will recreate database
```
...
```yaml
# .github/workflows/prltc-stats.yml
name: PRLTC Stats Report
on:
  schedule:
...
def send_prltc_stats():
    result = subprocess.run(['prltc', 'gain', '--format', 'json'],
                           capture_output=True, text=True)
    data = json.loads(result.stdout)

    message = f"""
    📊 *PRLTC Token Savings Report*
...
prltc gain --format json 2>&1 | tee /tmp/prltc-debug.log | jq .

# Use release build to avoid warnings
cargo build --release
./target/release/prltc gain --format json
```
...
# Validate estimation
prltc git status > output.txt
python -c "
import tiktoken
enc = tiktoken.get_encoding('cl100k_base')
text = open('output.txt').read()
print(f'Actual tokens: {len(enc.encode(text))}')
print(f'prltc estimate: {len(text) // 4}')
"
```

## Best Practices

1. **Regular Exports**: `prltc gain --all --format json > monthly-$(date +%Y%m).json`
