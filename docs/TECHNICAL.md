...
> **Start here** for a guided tour of how PRLTC works end-to-end. For the deep reference (filtering taxonomy, performance benchmarks, architecture decisions), see [ARCHITECTURE.md](../ARCHITECTURE.md).
>
...
PRLTC sits between the agent and the CLI, filtering outputs to keep only what matters. This achieves 60-90% token savings per command, reducing costs and increasing effective context window utilization. PRLTC is a single Rust binary with zero external dependencies, adding less than 10ms overhead per command.
...
4. `prltc rewrite` consults the command registry (70+ patterns) and returns `prltc git status`
...
| [`hooks/`](../src/hooks/README.md) | Hook system (8 files) | Installation flow (`prltc init`), integrity verification, rewrite command, trust model |
| [`analytics/`](../src/analytics/README.md) | Token savings analytics (4 files) | `prltc gain` dashboard, Claude Code economics, ccusage parsing |
...
PRLTC supports 7 LLM agents through hook integrations:
