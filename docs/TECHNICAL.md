# PRLTC Technical Documentation

> **Start here** for a guided tour of how PRLTC works end-to-end.
>
...
PRLTC sits between the agent and the CLI, filtering outputs to keep only what matters. This achieves 60-90% token savings per command, reducing costs and increasing effective context window utilization. PRLTC is a single Rust binary with no runtime dependencies beyond the compiled binary itself, adding less than 10ms overhead per command.
...
|  Intercepts: "git status" -> "prltc git status"    |
+-------------------------+------------------------+
                          |
                          v
+--------------------------------------------------+
...
- Exit code propagation: PRLTC never swallows non-zero exits
...
This is the full lifecycle of a command through PRLTC, from LLM agent to filtered output.

### 3.1 Hook Installation (`prltc init`)

The user runs `prltc init` to set up hooks for their LLM agent. This:

1. Writes a thin shell hook script (e.g., `~/.claude/hooks/prltc-rewrite.sh`)
...
4. Writes PRLTC awareness instructions (e.g., `PRLTC.md`) for prompt-level guidance

PRLTC supports 7 agents, each with its own installation mode. The hook scripts are embedded in the binary and written at install time.
...
3. The hook calls `prltc rewrite "git status"` as a subprocess
4. `prltc rewrite` consults the command registry and returns `prltc git status`
5. The hook sends a response telling the agent to use the rewritten command
6. If anything fails (jq missing, prltc not found, no match), the hook exits silently -- the raw command runs unchanged
...
> **Details**: [`hooks/README.md`](../hooks/README.md) covers each agent's JSON format, the rewrite registry, compound command handling, and the `PRLTC_DISABLED` override.

#### Rewrite Pipeline

The rewrite pipeline is how PRLTC intercepts and rewrites commands. The call chain is:
...
LLM Agent: "cargo fmt --all && cargo test 2>&1 | tail -20"
  |
  |  Hook shell (hooks/claude/prltc-rewrite.sh)
  |  Reads JSON from agent, extracts command, calls `prltc rewrite "$CMD"`
  |  On failure (jq missing, prltc missing, old version): exit 0 (passthrough)
  |
  v
rewrite_cmd::run(cmd)                              [src/hooks/rewrite_cmd.rs]
  ...
  |     → None → exit(1)          (no PRLTC equivalent, passthrough)
...
rewrite_command(cmd, excluded)                     [src/discover/registry.rs]
  ...
  |  - Simple "prltc ..." (no operators) → return as-is
...
rewrite_segment(seg, excluded)                     [src/discover/registry.rs]
  ...
  |
  |  Step 4 — Already PRLTC → return as-is
  |
  |  Step 5 — Special cases (short-circuit before classification)
  |  head -N / --lines=N → rewrite_line_range() → "prltc read file --max-lines N"
  |  tail -N / -n N / --lines N → rewrite_line_range() → "prltc read file --tail-lines N"
  ...
  |  c. Guard: PRLTC_DISABLED=1 in prefix → None
  |  d. Guard: gh with --json/--jq/--template → None
  |  e. Apply rule's rewrite_prefixes: "cargo fmt" → "prltc cargo fmt"
  |  f. Reassemble: env_prefix + prltc_cmd + args + redirect_suffix
  |
  v
classify_command(cmd)                              [src/discover/registry.rs]
  |  1. Check IGNORED_EXACT (cd, echo, fi, done, ...)
  |  2. Check IGNORED_PREFIXES (prltc, mkdir, mv, ...)
  ...
  |  9. Return Classification::Supported { prltc_equivalent, category, savings, status }
  |
  v
Result: "prltc cargo fmt --all && prltc cargo test 2>&1 | tail -20"
  |
  |  Hook response
...
LLM Agent executes rewritten command
  (bash handles && and |, each prltc invocation is a separate process)
```
...
Once the rewritten command reaches PRLTC:
...
1. Guard: check if the command is an PRLTC meta-command (`gain`, `init`, etc.) -- if so, show Clap error
...
Every command execution records metrics to SQLite (`~/.local/share/prltc/tracking.db`):
...
Analytics commands (`prltc gain`, `prltc cc-economics`, `prltc session`) query this database to produce dashboards and ROI reports.
...
1. Raw unfiltered output is saved to `~/.local/share/prltc/tee/{epoch}_{slug}.log`
...
| [`hooks/`](../src/hooks/README.md) | Hook system | Installation flow (`prltc init`), integrity verification, rewrite command, trust model |
| [`analytics/`](../src/analytics/README.md) | Token savings analytics | `prltc gain` dashboard, Claude Code economics, ccusage parsing |
...
PRLTC supports the following LLM agents through hook integrations:

| Agent | Hook Type | Mechanism | Can Modify Command? |
|-------|-----------|-----------|---------------------|
| Claude Code | Shell hook | `PreToolUse` in `settings.json` | Yes (`updatedInput`) |
| GitHub Copilot (VS Code) | Rust binary | `prltc hook copilot` reads JSON | Yes (`updatedInput`) |
| GitHub Copilot CLI | Rust binary | `prltc hook copilot` reads JSON | No (deny + suggestion) |
| Cursor | Shell hook | `preToolUse` hook | Yes (`updated_input`) |
| Gemini CLI | Rust binary | `prltc hook gemini` reads JSON | Yes (`hookSpecificOutput`) |
...
Declarative filters with an 8-stage pipeline: strip ANSI, regex replace, match output, strip/keep lines, truncate lines, head/tail, max lines, on-empty message. Loaded from three tiers: built-in (compiled), global (`~/.config/prltc/filters/`), project-local (`.prltc/filters/`, trust-gated).
...
|--------|--------|--------------|
| Startup time | < 10ms | `hyperfine 'prltc git status' 'git status'` |
| Memory usage | < 5MB resident | `/usr/bin/time -v prltc git status` |
| Binary size | < 5MB stripped | `ls -lh target/release/prltc` |
