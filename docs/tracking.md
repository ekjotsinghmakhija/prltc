# PRLTC Tracking API Documentation

Comprehensive documentation for PRLTC's token savings tracking system.
...
PRLTC's tracking system records every command execution to provide analytics on token savings. The system:
- Stores command history in SQLite (~/.local/share/prltc/tracking.db)
...
```
prltc command execution
  ↓
TimedExecution::start()
  ↓
[command runs]
  ↓
TimedExecution::track(original_cmd, prltc_cmd, input, output)
  ↓
Tracker::record(original_cmd, prltc_cmd, input_tokens, output_tokens, exec_time_ms)
  ↓
SQLite database (~/.local/share/prltc/tracking.db)
  ↓
Aggregation APIs (get_summary, get_all_days, etc.)
  ↓
CLI output (prltc gain) or JSON/CSV export
```

### Storage Location

- **Linux**: `~/.local/share/prltc/tracking.db`
- **macOS**: `~/Library/Application Support/prltc/tracking.db`
- **Windows**: `%APPDATA%\prltc\tracking.db`
...
impl Tracker {
    ...
    pub fn record(
        ...
        prltc_cmd: &str,            // PRLTC command (e.g., "prltc ls")
...
pub struct CommandRecord {
    ...
    pub prltc_cmd: String,           // PRLTC command used
...
```rust
use prltc::tracking::{TimedExecution, Tracker};
...
fn main() -> anyhow::Result<()> {
    ...
    let output = execute_prltc_command()?;
...
fn main() -> anyhow::Result<()> {
    ...
    for cmd in recent {
        println!("{}: {} saved {:.1}%",
            cmd.timestamp, cmd.prltc_cmd, cmd.savings_pct);
    }
...
```rust
use prltc::tracking::TimedExecution;
...
fn main() -> anyhow::Result<()> {
    ...
    timer.track_passthrough("git tag --list", "prltc git tag --list");
...
```yaml
# .github/workflows/track-prltc-savings.yml
name: Track PRLTC Savings
...
jobs:
  track-savings:
    ...
    steps:
      - name: Install PRLTC
        run: cargo install --git https://github.com/ekjotsinghmakhija/prltc

      - name: Export weekly stats
        run: |
          prltc gain --weekly --format json > prltc-weekly.json
          cat prltc-weekly.json

      - name: Upload artifact
        uses: actions/upload-artifact@v3
        with:
          name: prltc-metrics
          path: prltc-weekly.json
      ...
      - name: Post to Slack
        ...
        run: |
          SAVINGS=$(jq -r '.[0].saved_tokens' prltc-weekly.json)
          PCT=$(jq -r '.[0].savings_pct' prltc-weekly.json)
          curl -X POST -H 'Content-type: application/json' \
            --data "{\"text\":\"📊 PRLTC Weekly: ${SAVINGS} tokens saved (${PCT}%)\"}" \
            $SLACK_WEBHOOK
```
...
#!/usr/bin/env python3
"""
Export PRLTC metrics to Grafana/Datadog/etc.
"""
import json
import subprocess
from datetime import datetime

def get_prltc_metrics():
    """Fetch PRLTC metrics as JSON."""
...
def export_to_datadog(metrics):
    ...
    for day in metrics.get("daily", []):
        datadog.api.Metric.send(
            metric="prltc.tokens_saved",
            points=[(datetime.now().timestamp(), day["saved_tokens"])],
            tags=[f"date:{day['date']}"]
        )

        datadog.api.Metric.send(
            metric="prltc.savings_pct",
            points=[(datetime.now().timestamp(), day["savings_pct"])],
            tags=[f"date:{day['date']}"]
        )

if __name__ == "__main__":
    metrics = get_prltc_metrics()
    export_to_datadog(metrics)
    print(f"Exported {len(metrics.get('daily', []))} days to Datadog")
```

### Rust Integration (Using PRLTC as Library)

```rust
// In your Cargo.toml
// [dependencies]
// prltc = { git = "https://github.com/ekjotsinghmakhija/prltc" }

use prltc::tracking::{Tracker, TimedExecution};
use anyhow::Result;
...
CREATE TABLE commands (
    ...
    prltc_cmd TEXT NOT NULL,             -- PRLTC command (e.g., "prltc ls")
...
- Ensure only one PRLTC process writes at a time
- Check file permissions on `~/.local/share/prltc/tracking.db`
- Delete and recreate: `rm ~/.local/share/prltc/tracking.db && prltc gain`

### Missing exec_time_ms column

Older databases may not have the `exec_time_ms` column. PRLTC automatically migrates on first use, but you can force it:

```bash
sqlite3 ~/.local/share/prltc/tracking.db \
  "ALTER TABLE commands ADD COLUMN exec_time_ms INTEGER DEFAULT 0"
```
...
- [COMMAND_AUDIT.md](../claudedocs/COMMAND_AUDIT.md) - List of all PRLTC commands
