...
The LLM doesn't know PRLTC is involved for which commands, hooks rewrite commands silently. PRLTC's output must be a valid, useful subset of the original tool's output, not a different format the LLM wouldn't expect. If an LLM parses `git diff` output, PRLTC's filtered version must still look like `git diff` output.
...
- Others features not related to a LLM-proxy like PRLTC
