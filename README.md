...
```bash
prltc --version   # Should show "prltc 0.26.0"
...
> **Note**: prltc installs to `~/.local/bin` by default. If this directory is not in your PATH, add it:
> ```bash
...
```
  Claude Code types:  git status
                    ...
                    │  prltc-rewrite.sh              │
                    │  "git status"                │
                    │    →  "prltc git status"       │  transparent rewrite
                    └──────┬──────────────────────┘
                           │
                    ┌──────▼──────────────────────┐
                    │  PRLTC (Rust binary)           │
