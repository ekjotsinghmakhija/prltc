...
description: Review PRLTC Rust code for idiomatic simplification. Detects over-engineering, unnecessary allocations, verbose patterns. Applies Rust idioms without changing behavior.
triggers:
  - "simplify"
  - "too verbose"
  - "over-engineered"
...
Review and simplify Rust code in PRLTC while respecting the project's constraints.
...
// ✅ if let (but still handle errors in PRLTC — don't silently drop)
if let Ok(s) = output {
    process(&s);
}
// Note: in PRLTC filters, always handle Err with eprintln! + fallback
```

## PRLTC-Specific Checks
