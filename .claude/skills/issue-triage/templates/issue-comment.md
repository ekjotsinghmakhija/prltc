...
- {Specific missing info 1 — e.g., "PRLTC version (`prltc --version` output)"}
...
---
*Triaged via [prltc](https://github.com/ekjotsinghmakhija/prltc) `/issue-triage`*
```
...
Use when: issue requests something that doesn't align with PRLTC's design goals (e.g., adding async runtime, platform-specific features outside scope, changing core behavior).

```markdown
## Closing: Out of Scope

After review, this request falls outside PRLTC's current design goals.

### Rationale

{1-2 sentences explaining why — be specific. Reference design constraints if relevant, e.g., "PRLTC is intentionally single-threaded with zero async dependencies to maintain <10ms startup time."}

### Alternatives

{If applicable: what the user can do instead. E.g., "For this use case, `prltc proxy <cmd>` gives you raw output while still tracking usage metrics."}
...
**PRLTC-specific context to include when relevant** :
- Mention `prltc --version` as the first diagnostic step for bug reports
