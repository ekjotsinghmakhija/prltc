...
Systematic performance analysis and optimization for PRLTC CLI tool, focusing on **startup time (<10ms)**, **memory usage (<5MB)**, and **token savings (60-90%)**.
...
## PRLTC Performance Targets

| Metric | Target | Verification Method | Failure Threshold |
|--------|--------|---------------------|-------------------|
| **Startup time** | <10ms | `hyperfine 'prltc <cmd>'` | >15ms = blocker |
| **Memory usage** | <5MB resident | `/usr/bin/time -l prltc <cmd>` (macOS) | >7MB = blocker |
| **Token savings** | 60-90% | Tests with `count_tokens()` | <60% = blocker |
| **Binary size** | <5MB stripped | `ls -lh target/release/prltc` | >8MB = investigate |
...
hyperfine 'prltc git status' --warmup 3 --export-json /tmp/baseline_startup.json

# Memory usage baseline (macOS)
/usr/bin/time -l prltc git status 2>&1 | grep "maximum resident set size" > /tmp/baseline_memory.txt

# Memory usage baseline (Linux)
/usr/bin/time -v prltc git status 2>&1 | grep "Maximum resident set size" > /tmp/baseline_memory.txt

# Binary size baseline
ls -lh target/release/prltc | tee /tmp/baseline_binary_size.txt
```
...
hyperfine 'target/release/prltc git status' --warmup 3 --export-json /tmp/after_startup.json

# Measure memory usage
/usr/bin/time -l target/release/prltc git status 2>&1 | grep "maximum resident set size" > /tmp/after_memory.txt

# Check binary size
ls -lh target/release/prltc | tee /tmp/after_binary_size.txt
```
...
hyperfine 'prltc git status' 'target/release/prltc git status' --warmup 3

# Example output:
#   Benchmark 1: prltc git status
#     Time (mean ± σ):       6.2 ms ±   0.3 ms    [User: 4.1 ms, System: 1.8 ms]
#   Benchmark 2: target/release/prltc git status
#     Time (mean ± σ):       7.8 ms ±   0.4 ms    [User: 5.2 ms, System: 2.1 ms]
#
#   Summary
#     'prltc git status' ran 1.26 times faster than 'target/release/prltc git status'
...
cargo flamegraph -- target/release/prltc git status
...
cargo flamegraph -- target/release/prltc git log -10
...
valgrind --tool=dhat target/release/prltc git status
```
...
```bash
# strace on Linux
strace -c target/release/prltc git status 2>&1 | grep -E "open|read"

# dtrace on macOS (requires SIP disabled)
sudo dtrace -n 'syscall::open*:entry { @[execname] = count(); }' &
target/release/prltc git status
sudo pkill dtrace
```
...
fn main() {
    let config = load_config().unwrap(); // Reads ~/.config/prltc/config.toml
    // ...
}
...
### Binary Size
- [ ] Check `ls -lh target/release/prltc`
...
CURRENT_TIME=$(hyperfine 'prltc git status' --warmup 3 --export-json /tmp/perf.json 2>&1 | grep "Time (mean" | awk '{print $4}')
...
# Check binary size
BINARY_SIZE=$(ls -l target/release/prltc | awk '{print $5}')
...
- name: Performance Regression Check
  run: |
    ...
    # Benchmark startup time
    hyperfine 'target/release/prltc git status' --warmup 3 --max-runs 10
...
|------|---------|---------|
| **hyperfine** | Benchmark startup time | `hyperfine 'prltc <cmd>' --warmup 3` |
| **time** | Memory usage (macOS) | `/usr/bin/time -l prltc <cmd>` |
| **time** | Memory usage (Linux) | `/usr/bin/time -v prltc <cmd>` |
| **flamegraph** | CPU profiling | `cargo flamegraph -- prltc <cmd>` |
...
| **strace** | System call tracing (Linux) | `strace -c target/release/prltc <cmd>` |
