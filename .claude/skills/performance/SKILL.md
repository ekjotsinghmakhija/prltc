---
name: performance
description: PRLTC CLI performance analysis and optimization. Startup time (<10ms), binary size (<5MB), regex compilation, memory usage. Use when adding dependencies, changing initialization, or suspecting regressions.
triggers:
  - "startup time"
...
hyperfine 'prltc git status' --warmup 3 --export-json /tmp/before.json

# After changes — rebuild first
cargo build --release

# Compare against installed
hyperfine 'target/release/prltc git status' 'prltc git status' --warmup 3
...
```bash
# macOS
/usr/bin/time -l target/release/prltc git status 2>&1 | grep "maximum resident"
# Target: <5,000,000 bytes (5MB)

# Linux
/usr/bin/time -v target/release/prltc git status 2>&1 | grep "Maximum resident"
# Target: <5,000 kbytes
```
...
# Measure after
hyperfine 'target/release/prltc git status' --warmup 3
...
instruments -t Allocations target/release/prltc git log -10
...
Common PRLTC allocation hotspots:
...
# 1. Baseline
cargo build --release
hyperfine 'target/release/prltc git status' --warmup 5 --export-json /tmp/before.json
/usr/bin/time -l target/release/prltc git status 2>&1 | grep "maximum resident"
ls -lh target/release/prltc
...
cargo build --release
hyperfine 'target/release/prltc git status' --warmup 5 --export-json /tmp/after.json
/usr/bin/time -l target/release/prltc git status 2>&1 | grep "maximum resident"
ls -lh target/release/prltc
