...
Systematic release workflow for PRLTC: build verification, version bump, changelog update, git tag, and push to trigger CI/CD.
...
/usr/bin/time -l target/release/prltc git status
...
- **MAJOR** (v1.0.0): Breaking changes (rare for PRLTC)
- **MINOR** (v0.X.0): New features, new filters, new commands
- **PATCH** (v0.0.X): Bug fixes, performance improvements

**Examples**:
- New filter added (`prltc pytest`) → **MINOR** bump (v0.16.0 → v0.17.0)
...
### Added
- `prltc pytest` command for Python test filtering (90% token reduction)
...
Added:
- prltc pytest command (90% token reduction)
...
curl -sSL https://github.com/ekjotsinghmakhija/prltc/releases/download/v0.17.0/prltc-macos-latest -o prltc
chmod +x prltc
./prltc --version
# Should show v0.17.0
```
...
target/release/prltc --version | grep "$NEW_VERSION"
...
**Symptom**: `prltc --version` shows old version after bump

**Solution**:
```bash
# Cargo.lock might be out of sync
cargo update -p prltc
cargo build --release
