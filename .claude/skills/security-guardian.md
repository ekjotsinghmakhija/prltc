---
description: CLI security expert for PRLTC - command injection, shell escaping, hook security
---

# Security Guardian

Comprehensive security analysis for PRLTC CLI tool, focusing on **command injection**, **shell escaping**, **hook security**, and **malicious input handling**.
...
## PRLTC Security Threat Model

PRLTC faces unique security challenges as a CLI proxy that:
1. **Executes shell commands** based on user input
2. **Parses untrusted command output** (git, cargo, gh, etc.)
3. **Integrates with Claude Code hooks** (prltc-rewrite.sh, prltc-suggest.sh)
...
| **Malicious Output** | 🟡 MEDIUM | PRLTC crash, DoS | Robust parsing, error handling |
...
# Validate PRLTC binary exists and is executable
if ! command -v prltc >/dev/null 2>&1; then
    echo "Error: prltc binary not found"
    exit 1
fi
...
fn test_malicious_output_handling() {
    // Malformed outputs that could crash PRLTC
...
# 2. Verify PRLTC binary exists
if ! command -v prltc >/dev/null 2>&1; then
    exit 1
fi

# 3. Use absolute path (prevent PATH hijacking)
PRLTC_BIN=$(which prltc)

# 4. Validate PRLTC version (prevent downgrade attacks)
if ! "$PRLTC_BIN" --version | grep -q "prltc 0.16"; then
    echo "Warning: PRLTC version mismatch"
fi
...
```markdown
## Security Advisory: Command Injection in prltc v0.16.0
...
**Description**:
PRLTC versions 0.15.0 through 0.16.0 are vulnerable to command injection
...
**Mitigation**:
Upgrade to v0.16.1 immediately. As a workaround, avoid using PRLTC in
