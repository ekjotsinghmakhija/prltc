---
model: sonnet
description: PRLTC Codebase Health Audit — 7 catégories scorées 0-10
argument-hint: "[--category <cat>] [--fix] [--json]"
allowed-tools: [Read, Grep, Glob, Bash, Write]
---

# Audit Codebase — Santé du Projet PRLTC
...
# Dépendances async (interdit dans PRLTC)
Grep "tokio\|async-std\|futures" Cargo.toml

# Taille binaire post-strip
ls -lh target/release/prltc 2>/dev/null || echo "Build needed"
```
...
**Objectif** : Architecture PRLTC respectée, conventions Rust appliquées.
...
which hyperfine && hyperfine 'prltc git status' --warmup 3 2>&1 | grep "Time"

# Mémoire binaire
ls -lh target/release/prltc 2>/dev/null
