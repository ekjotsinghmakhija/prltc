...
PRLTC is a **zero-overhead CLI proxy**. Every architectural decision must be evaluated against:
...
```
~/.config/prltc/filters/    ← User-global filters
<project>/.prltc/filters/   ← Project-local filters (shadow warning)
```

## Architectural Patterns (PRLTC Idioms)
...
**Will not:**
- Implement the full filter logic (→ rust-prltc agent)
