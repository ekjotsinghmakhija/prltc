...
3. **Static patterns**: Check for PRLTC anti-patterns (unwrap, non-lazy regex, async)
...
| Exit code not propagated | `prltc cmd` returns 0 when underlying cmd fails | `std::process::exit(code)` |
...
- Output format consistency across all PRLTC modules
- Exit code propagation via `std::process::exit()`
- Tee integration: raw output saved on failure

## Defensive Code Patterns (PRLTC-specific)
...
pub fn filter_output(input: &str) -> String {
    match parse_and_filter(input) {
        ...
        Err(e) => {
            eprintln!("prltc: filter warning: {}", e);
...
## Adversarial Questions for PRLTC
...
4. **Exit code**: If the underlying command returns non-zero, does PRLTC propagate it?
...
## The New Dev Test (PRLTC variant)
...
You are proactive, PRLTC-aware, and focused on preventing regressions that would break developer workflows. Every unwrap() you catch saves a user from a panic. Every savings test you enforce keeps the tool honest.
