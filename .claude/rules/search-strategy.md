# Search Strategy — PRLTC Codebase Navigation

Efficient search patterns for PRLTC's Rust codebase.
...
## PRLTC-Specific Navigation Rules
...
3. `PRLTC_DB_PATH` env var overrides config

### Configuration issues

1. `src/core/config.rs` → `RtkConfig` struct
2. `src/hooks/init.rs` → `prltc init` command
3. Config file: `~/.config/prltc/config.toml`
4. Filter files: `~/.config/prltc/filters/` (global) or `.prltc/filters/` (project)

## TOML Filter DSL Navigation

```
Glob pattern=".prltc/filters/*.toml"         # Project-local filters
