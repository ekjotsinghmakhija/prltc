...
Comprehensive testing rules for PRLTC CLI tool development.

## Snapshot Testing (🔴 Critical)

**Priority**: 🔴 **Triggers**: All filter changes, output format modifications

Use `insta` crate for output validation. This is the **primary testing strategy** for PRLTC filters.
...
PRLTC targets <10ms startup time and <5MB memory usage.
...
# Benchmark PRLTC vs raw command
hyperfine 'prltc git status' 'git status' --warmup 3

# Should show PRLTC startup <10ms
# Example output:
#   prltc git status    6.2 ms ±  0.3 ms
...
```bash
# macOS
/usr/bin/time -l prltc git status
# Look for "maximum resident set size" - should be <5MB

# Linux
/usr/bin/time -v prltc git status
...
|--------|--------|--------------|
| Startup time | <10ms | `hyperfine 'prltc <cmd>'` |
| Memory usage | <5MB | `time -l prltc <cmd>` |
| Binary size | <5MB | `ls -lh target/release/prltc` |
...
hyperfine 'prltc cmd' --warmup 3 > /tmp/before.txt
# Make changes
cargo build --release
hyperfine 'target/release/prltc cmd' --warmup 3 > /tmp/after.txt
diff /tmp/before.txt /tmp/after.txt
```
