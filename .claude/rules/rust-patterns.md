# Rust Patterns — PRLTC Development Rules

PRLTC-specific Rust idioms and constraints. Applied to all code in this repository.

## Non-Negotiable PRLTC Rules
...
Note: `lazy_static!` with `.unwrap()` for initialization is the **established PRLTC pattern** — it's acceptable because a bad regex literal is a programming error caught at first use.
...
// ❌ Silent swallow — catastrophic in PRLTC (user gets no output)
match result {
    Ok(output) => process(output),
    Err(_) => {}
}
