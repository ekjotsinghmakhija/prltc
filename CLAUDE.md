...
prltc cargo clippy --all-targets # preferred
```
...
prltc uses a **command proxy architecture**: `main.rs` routes CLI commands via a Clap `Commands` enum to specialized filter modules in `src/cmds/*/`, each of which executes the underlying command and compresses its output. Token savings are tracked in SQLite via `src/core/tracking.rs`.
...
**Examples**:
```bash
prltc proxy git log --oneline -20    # Full git log output (no truncation)
prltc proxy npm install express      # Raw npm output (no filtering)
prltc proxy curl https://api.example.com/data  # Any command works
```

All proxy commands appear in `prltc gain --history` with 0% savings (input = output).
...
```bash
hyperfine 'prltc git log -10' --warmup 3          # before
cargo build --release
hyperfine 'target/release/prltc git log -10' --warmup 3  # after (should be <10ms)
```
