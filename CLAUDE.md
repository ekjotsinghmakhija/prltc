...
```bash
prltc --version  # Should show "prltc 0.28.2" (or newer)
...
- Saves raw unfiltered output to `~/.local/share/prltc/tee/` on command failure
- Prints one-line hint `[full output: ~/.local/share/prltc/tee/...]` so LLMs can read instead of re-run
- Configurable via `[tee]` section in config.toml or env vars (`PRLTC_TEE`, `PRLTC_TEE_DIR`)
...
PRLTC has **strict performance targets** to maintain zero-overhead CLI experience:

| Metric | Target | Verification Method |
|--------|--------|---------------------|
| **Startup time** | <10ms | `hyperfine 'prltc git status' 'git status'` |
| **Memory overhead** | <5MB resident | `/usr/bin/time -l prltc git status` (macOS) |
...
```bash
# Before changes
hyperfine 'prltc git log -10' --warmup 3 > /tmp/before.txt

# After changes
cargo build --release
hyperfine 'target/release/prltc git log -10' --warmup 3 > /tmp/after.txt

# Compare (should be <10ms)
diff /tmp/before.txt /tmp/after.txt
```

**Why <10ms matters**: Claude Code users expect CLI tools to be instant. Any perceptible delay (>10ms) breaks the developer flow. PRLTC achieves this through:
...
PRLTC follows Rust best practices for error handling:

**Rules**:
- **anyhow::Result** for CLI binary (PRLTC is an application, not a library)
...
- PRLTC is single-threaded by design
...
- `prltc git status | grep modified` must work
...
- **Python Commands**: 3 commands for Python development workflows
  - `prltc ruff check/format`: Ruff linter/formatter with JSON (check) and text (format) parsing (80%+ reduction)
  - `prltc pytest`: Pytest test runner with state machine text parser (90%+ reduction)
  - `prltc pip list/outdated/install`: pip package manager with auto-detect uv (70-85% reduction)
- **Go Commands**: 4 commands via sub-enum for Go ecosystem
  - `prltc go test`: NDJSON line-by-line parser for interleaved events (90%+ reduction)
  - `prltc go build`: Text filter showing errors only (80% reduction)
  - `prltc go vet`: Text filter for issues (75% reduction)
  - `prltc golangci-lint`: JSON parsing grouped by rule (85% reduction)
...
- **Ruby Commands**: 3 modules for Ruby/Rails development
  - `prltc rspec`: RSpec test runner with JSON injection (`--format json`), text fallback (60%+ reduction)
  - `prltc rubocop`: RuboCop linter with JSON injection, group by cop/severity (60%+ reduction)
  - `prltc rake test`: Minitest filter via rake/rails test, state machine parser (85-90% reduction)
...
**Why**: PRLTC is a production CLI tool used by developers in their workflows. Bugs break developer productivity. Quality gates prevent regressions and maintain user trust.

**Performance verification** (for filter changes):

```bash
# Benchmark before/after
hyperfine 'prltc git log -10' --warmup 3
cargo build --release
hyperfine 'target/release/prltc git log -10' --warmup 3

# Memory profiling
/usr/bin/time -l target/release/prltc git status  # macOS
/usr/bin/time -v target/release/prltc git status  # Linux
```
...
- **For new filters**: Test with real command (`prltc <cmd>`), verify output matches expectations
  - Example: `prltc git log -10` → inspect output, verify condensed correctly
  - Example: `prltc cargo test` → verify only failures shown, not full output
...
- **For hook changes**: Test in real Claude Code session, verify command rewriting works
  ...
  - Verify hook rewrites to `prltc git status`

- **For performance**: Run `hyperfine` comparison (before/after), verify <10ms startup
  - Benchmark baseline: `hyperfine 'prltc git status' --warmup 3`
  - Make changes, rebuild
  - Benchmark again: `hyperfine 'target/release/prltc git status' --warmup 3`
...
- **For cross-platform**: Test on macOS + Linux (Docker) + Windows (CI), verify shell escaping
  ...
  - Linux (bash): Use Docker `docker run --rm -v $(pwd):/prltc -w /prltc rust:latest cargo test`
  - Windows (PowerShell): Trust CI/CD pipeline or test manually if available

**Anti-pattern**: Running only automated tests (`cargo test`, `cargo clippy`) without actually executing `prltc <cmd>` and inspecting output.

**Example**: If fixing the `git log` filter, run `prltc git log -10` and verify:
...
```bash
pwd  # Verify you're in the prltc project root
...
When adding a new filter (e.g., `prltc newcmd`):
...
- [ ] Test manually: `prltc <cmd>` and inspect output for correctness
...
**Example workflow** (adding `prltc newcmd`):
