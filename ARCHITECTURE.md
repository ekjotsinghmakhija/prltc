# prltc Architecture Documentation

> **prltc (Rust Token Killer)** - A high-performance CLI proxy that minimizes LLM token consumption through intelligent output filtering and compression.

This document provides a comprehensive architectural overview of prltc, including system design, data flows, module organization, and implementation patterns.
...
```
┌────────────────────────────────────────────────────────────────────────┐
│                      prltc - Token Optimization Proxy                    │
└────────────────────────────────────────────────────────────────────────┘

User Input          CLI Layer           Router            Module Layer
──────────          ─────────           ──────            ────────────

$ prltc git log    ─→  Clap Parser   ─→   Commands    ─→   git::run()
...
Files:
  ~/.claude/hooks/prltc-rewrite.sh  ← thin delegator (calls `prltc rewrite`, ~50 lines)
  ~/.claude/settings.json         ← hook registry (PreToolUse registration)
  ~/.claude/PRLTC.md                ← minimal context hint (10 lines)
```
...
tracking::track(
    ...
    prltc_cmd: "prltc git log --oneline -5",
...
Database: ~/.local/share/prltc/history.db
```
...
- Natural CLI: `prltc go test` not `prltc gotest`
...
Command                 Raw Time    prltc Time    Overhead    Savings
─────────────────────────────────────────────────────────────────────────
...
├─────────┼──────────────────────────────────────────────────────┤
│ 0       │ Success                                              │
│ 1       │ prltc internal error (parsing, filtering, etc.)        │
...
1. User Settings (config.toml)
   ───────────────────────────
   Location: ~/.config/prltc/config.toml
...
2. LLM Integration (CLAUDE.md)
   ────────────────────────────
   Locations:
   • Global: ~/.config/prltc/CLAUDE.md
   • Local:  ./CLAUDE.md (project-specific)

   Purpose: Instruct LLM (Claude Code) to use prltc prefix
   Created by: prltc init [--global]

   Template (init.rs:40-60):
   # CLAUDE.md
   Use `prltc` prefix for all commands:
   - prltc git status
   - prltc grep "pattern"
   - prltc read file.rs
...
Check existing CLAUDE.md:
  ...
  • else      → ./CLAUDE.md
      ...
      └─ Not exists? → Continue
      ↓
Prompt: "Initialize prltc for LLM usage? [y/N]"
      ↓ Yes
Write template:
┌─────────────────────────────────────┐
│ # CLAUDE.md                         │
│                                     │
│ Use `prltc` prefix for commands:      │
│ - prltc git status                    │
│ - prltc lint                          │
│ - prltc test                          │
│                                     │
...
└─────────────────────────────────────┘
      ↓
Success: "✓ Initialized prltc for LLM integration"
```
...
┌──────────────────────┬──────────────┬──────────────┐
│ Operation            │ prltc Overhead │ Total Time   │
├──────────────────────┼──────────────┼──────────────┤
│ prltc git status       │ +8ms         │ 58ms         │
│ prltc grep "pattern"   │ +12ms        │ 145ms        │
│ prltc read file.rs     │ +5ms         │ 15ms         │
│ prltc lint             │ +15ms        │ 2.5s         │
└──────────────────────┴──────────────┴──────────────┘
...
**Step-by-step process to add a new prltc command:**
...
pub fn run(args: &[String], verbose: u8) -> Result<()> {
    ...
    // Track savings
    tracking::track("mycmd", "prltc mycmd", &raw, &filtered);
...
# Build and test
cargo build
./target/debug/prltc mycmd arg1 arg2
...
**prltc mycmd** - Description of what it does
...
| **Proxy Pattern** | prltc sits between user and tool, transforming output |
