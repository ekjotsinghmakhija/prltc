...
> **Deep reference** for PRLTC's system design, filtering taxonomy, performance characteristics, and architecture decisions. For a guided tour of the end-to-end flow, start with [docs/TECHNICAL.md](docs/TECHNICAL.md).

**prltc (Rust Token Killer)** is a high-performance CLI proxy that minimizes LLM token consumption through intelligent output filtering and compression.
...
Two tiers: **User settings** (`~/.config/prltc/config.toml`) and **LLM integration** (CLAUDE.md via `prltc init`).
