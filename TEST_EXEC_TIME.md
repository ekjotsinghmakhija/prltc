...
prltc git log -10          # Fast (~10ms)
prltc cargo test           # Slow (~300ms)
prltc vitest run           # Very slow (seconds)
...
**JSON Export:**
```bash
prltc gain --daily --format json | jq '.summary'
...
for i in {1..10}; do prltc git status; done
...
- [ ] `prltc gain` shows "Total exec time: X (avg Yms)"
- [ ] By Command table has "Time" column
- [ ] `prltc gain --daily` shows time per day
...
sqlite3 ~/.local/share/prltc/history.db "PRAGMA table_info(commands);"
