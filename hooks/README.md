...
**Deployed hook artifacts** — the actual files installed on user machines by `prltc init`. These are shell scripts, TypeScript plugins, and rules files that run outside the Rust binary. They are **thin delegates**: parse agent-specific JSON, call `prltc rewrite` as a subprocess, format agent-specific response. Zero filtering logic lives here.
...
LLM agent integrations that intercept CLI commands and route them through PRLTC for token optimization. Each hook transparently rewrites raw commands (e.g., `git status`) to their PRLTC equivalents (e.g., `prltc git status`), delivering 60-90% token savings without requiring the agent or user to change their workflow.
...
Agent runs command (e.g., "cargo test --nocapture")
  ...
  -> Calls `prltc rewrite "cargo test --nocapture"`
  -> Registry matches pattern, returns "prltc cargo test --nocapture"
  -> Hook sends response in agent-specific JSON format
  -> Agent executes "prltc cargo test --nocapture" instead
  -> Filtered output reaches LLM (~90% fewer tokens)
```

All rewrite logic lives in the Rust binary (`src/discover/registry.rs`). Hook scripts are **thin delegates** that handle agent-specific JSON formats and call `prltc rewrite` for the actual decision. This ensures a single source of truth for all 70+ rewrite patterns.
...
| VS Code Copilot Chat | Rust binary (`prltc hook copilot`) | Transparent rewrite | Yes (`updatedInput`) |
| GitHub Copilot CLI | Rust binary (`prltc hook copilot`) | Deny-with-suggestion | No (agent retries) |
| Cursor | Shell hook (`preToolUse`) | Transparent rewrite | Yes (`updated_input`) |
| Gemini CLI | Rust binary (`prltc hook gemini`) | Transparent rewrite | Yes (`hookSpecificOutput`) |
...
```json
{
  "hookSpecificOutput": {
    ...
    "updatedInput": { "command": "prltc git status" }
  }
}
```
...
{
  "permission": "allow",
  "updated_input": { "command": "prltc git status" }
}
```
...
```json
{
  ...
  "permissionDecisionReason": "Token savings: use `prltc git status` instead"
}
```
...
```json
{
  "decision": "allow",
  "hookSpecificOutput": {
    "tool_input": { "command": "prltc git status" }
  }
}
```
...
Example: `cargo fmt --all && cargo test` becomes `prltc cargo fmt --all && prltc cargo test`

### Override Controls

- **`PRLTC_DISABLED=1`**: Per-command override (`PRLTC_DISABLED=1 git status` runs raw)
- **`exclude_commands`**: In `~/.config/prltc/config.toml`, list commands to never rewrite
- **Already-PRLTC**: `prltc git status` passes through unchanged (no `prltc prltc git`)
...
- prltc binary not found: warning to stderr, exit 0
- prltc version too old (< 0.23.0): warning to stderr, exit 0
- Invalid JSON input: pass through unchanged
- `prltc rewrite` crashes: hook exits 0 (subprocess error ignored)
...
PRLTC supports AI coding assistants that developers actually use day-to-day. To add a new agent:
